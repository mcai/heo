...
def generate_plot(csv_file_name, plot_file_name, x, hue, y, xticklabels_rotation=90, plot_type=BAR_PLOT):
    ...
    palette = sns.color_palette("Set1", 8) if hue else sns.color_palette(["#34495e"])

    ax = plot_func(data=df, x=x, hue=hue, y=y, palette=palette)
    ax.set_xlabel(x)
    ax.set_ylabel(y)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--xticklabels_rotation', type=int, default=90)
    arg_parser.add_argument('--plot_type', type=int, default=BAR_PLOT)
    ...
    generate_plot(
        ...
        hue=args.hue,
        y=args.y,
        xticklabels_rotation=args.xticklabels_rotation,
